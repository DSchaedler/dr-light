...
def tick(args)
  ...
  args.outputs.solids << args.state.box_collision_one if args.state.box_collision_one

  args.outputs.solids << args.state.box_collision_two if args.state.box_collision_two

  args.outputs.labels << if args.state.box_collision_one && args.state.box_collision_two
                           if args.state.box_collision_one.intersect_rect? args.state.box_collision_two
                             small_label(args, x, 4, 'The boxes intersect.')
                           else
                             small_label(args, x, 4, 'The boxes do not intersect.')
                           end
                         else
                           small_label(args, x, 4, '--')
                         end
end
