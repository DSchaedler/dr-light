...
def calc(args)
  ...
  if args.state.tick_count.mod_zero?(args.state.enemy_jump_interval) && args.state.enemy.dy.zero?
  ...
  args.state.enemy.hammers = args.state.enemy.hammers.reject { |h| (h[:y]).negative? }
  ...
  reset_player args if args.state.enemy.hammers.any? { |h| h[:rect].intersect_rect?(args.state.player.rect) }

  # if the enemy's rect intersects with (or hits) the player,
  # the reset_player method is called (so the game can start over)
  reset_player args if args.state.enemy.rect.intersect_rect? args.state.player.rect
end
