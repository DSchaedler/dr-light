...
def tick(args)
  ...
  args.outputs.labels << if args.state.last_mouse_click
                           if args.state.last_mouse_click.point.inside_rect? box
                             small_label(args, x, 16, 'Mouse click happened *inside* the box.')
                           else
                             small_label(args, x, 16, 'Mouse click happened *outside* the box.')
                           end
                         else
                           small_label(args, x, 16, 'Mouse click has not occurred yet.')
                         end
end
...
def tick_instructions(args, text, y = 715)
  ...
  args.outputs.debug << { x: 640, y: y - 25, text: '(click to dismiss instructions)', size_enum: -2, alignment_enum: 1,
