...
def tick(args)
  ...
  args.outputs.labels << { x: 460, y: row_to_px(args, 0), text: "Current game time: #{args.state.tick_count}",
                           size_enum: -1 }
  ...
  args.state.h_pressed_at = args.state.tick_count if args.inputs.keyboard.key_up.h
...
def tick_help_text(args)
  ...
  if args.inputs.keyboard.key_down.truthy_keys.length.positive? ||
     args.inputs.keyboard.key_held.truthy_keys.length.positive? ||
     args.inputs.keyboard.key_up.truthy_keys.length.positive?
  ...
  unless args.state.help_available
    args.outputs.labels << [10, row_to_px(args, 7),
                            "Press a key and I'll show code to access the key and what value will be returned if you used the code.", small_font]
    return
  end
...
def fill_single_history(args, history_key, _state_key, keyboard_method, key_name)
  current_value = args.inputs.keyboard.send(key_name)
  current_value = args.inputs.keyboard.send(keyboard_method).send(key_name) if keyboard_method
...
def render_help_labels(args, history_key, _state_key, keyboard_method, x)
  idx = 8
  args.outputs.labels << args.state
    ...
    current_value = args.inputs.keyboard.send(keyboard_method).send(k) if keyboard_method
    idx += 2
    [
