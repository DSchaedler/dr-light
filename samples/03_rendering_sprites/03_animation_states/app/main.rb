...
class Game
  ...
  def defaults
    state.show_debug_layer  = true if state.tick_count.zero?
    player.tile_size        = 64
    player.speed            = 3
  ...
  def sprite_horizontal_run
    ...
    tile_index = 0 unless player.is_moving

    {
      ...
      flip_horizontally: player.dir_x.positive?
      # a: 40
    }
  end
  ...
  def render_player
    outputs.sprites << if player.slash_at
                         sprite_horizontal_slash
                       elsif player.is_moving
                         sprite_horizontal_run
                       else
                         sprite_horizontal_stand
                       end
  end
  ...
  def render_debug_layer
    return unless state.show_debug_layer
  ...
  def calc_slash
    ...
    player.slash_collision_rect = if player.dir_x.positive?
                                    [player.x + player.tile_size,
  ...
  def calc
    ...
    add_enemy if state.enemies.length.zero?
    calc_movement
    calc_slash
  end
